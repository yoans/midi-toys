...
const s = function (sketch) {
  ...
  sketch.draw = function () {
    ...
    // non-rotated arrows
    const arrowsToNotRotateDictionary = Object.keys(arrowLocationDictionary).reduce((acc, location) => (arrowLocationDictionary[location].length === 1 ?
      [
        ...acc,
    ...
    const arrowsToRotateDictionary = Object.keys(arrowLocationDictionary).reduce((acc, location) => (arrowLocationDictionary[location].length !== 1 ?
      {
        ...acc,
  ...
  render() {
    ...
    return (
      <div className="midi-toys-app">
        ...
        max={maxArrows}
        min={minArrows}
        value={this.state.numberOfArows}
        onChange={this.newNumberOfArrowsHandler}/> */}
...
function changeMIDIOut(ev) {
  try {
    ...
    const outputArray = midiAccess.outputs.values().filter(output => selectedID === output.id);
    midiOut = outputArray.length ? outputArray[0] : null;
  } catch (err) {
